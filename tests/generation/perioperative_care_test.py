...
class TestCodeGenerationPerioperativeCare(unittest.TestCase):
    ...
    def test_parse_fixed_price_request(self):
        self.program = parse_program(
            Path(__file__).parent / ".wappl" / "perioperative-care.wappl")
        ...
        with open(Path(__file__).parent / ".results" / "generated" / "perioperative_care.py", "w") as f:
